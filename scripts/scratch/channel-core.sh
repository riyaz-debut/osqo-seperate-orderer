...
updateAnchorPeers(){
	...
	setOrgPeersGlobals $ORG $PEER
	set -x
	peer channel update -o  orderer0.osqo.osqo.com:$ORDERER_1_OSQO -c $CHANNEL_NAME --ordererTLSHostnameOverride orderer0.osqo.osqo.com -f ../channel-artifacts/$1MSPanchors.tx --tls --cafile $ORDERER_CA
	res=$?
  	{ set +x; } 2>/dev/null
	echo "==========================="
